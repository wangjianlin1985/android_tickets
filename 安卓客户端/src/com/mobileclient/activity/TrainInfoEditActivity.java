...
public class TrainInfoEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.traininfo_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("编辑车次信息信息");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_trainNumber.getText().toString().equals("")) {
						Toast.makeText(TrainInfoEditActivity.this, "车次输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_price.getText().toString().equals("")) {
						Toast.makeText(TrainInfoEditActivity.this, "票价输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_seatNumber.getText().toString().equals("")) {
						Toast.makeText(TrainInfoEditActivity.this, "总座位数输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_leftSeatNumber.getText().toString().equals("")) {
						Toast.makeText(TrainInfoEditActivity.this, "剩余座位数输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_startTime.getText().toString().equals("")) {
						Toast.makeText(TrainInfoEditActivity.this, "开车时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_endTime.getText().toString().equals("")) {
						Toast.makeText(TrainInfoEditActivity.this, "终到时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_totalTime.getText().toString().equals("")) {
						Toast.makeText(TrainInfoEditActivity.this, "历时输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传车次信息信息*/
					TrainInfoEditActivity.this.setTitle("正在更新车次信息信息，稍等...");
					String result = trainInfoService.UpdateTrainInfo(trainInfo);
	...
	private void initViewData() {
		...
		this.ET_trainNumber.setText(trainInfo.getTrainNumber());
		for (int i = 0; i < stationInfoList.size(); i++) {
			if (trainInfo.getStartStation() == stationInfoList.get(i).getStationId()) {
				this.spinner_startStation.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < stationInfoList.size(); i++) {
			if (trainInfo.getEndStation() == stationInfoList.get(i).getStationId()) {
		...
		for (int i = 0; i < seatTypeList.size(); i++) {
			if (trainInfo.getSeatType() == seatTypeList.get(i).getSeatTypeId()) {
				this.spinner_seatType.setSelection(i);
				break;
			}
		}
		this.ET_price.setText(trainInfo.getPrice() + "");
		this.ET_seatNumber.setText(trainInfo.getSeatNumber() + "");
		this.ET_leftSeatNumber.setText(trainInfo.getLeftSeatNumber() + "");
		this.ET_startTime.setText(trainInfo.getStartTime());
		this.ET_endTime.setText(trainInfo.getEndTime());
		this.ET_totalTime.setText(trainInfo.getTotalTime());
	}
	@Override
